...
func TestOnboard_OrgLocationMenuPublish(t *testing.T) {
	...
	// 2. Location
	locID := seedLocation(t, pool, orgID, "Main Branch "+suffix)
	...
	// Assert location row links to org
	var locOrgID string
	err := db.Scoped(ctx, pool, db.ServiceRoleScope(), func(tx pgx.Tx) error {
		return tx.QueryRow(ctx,
			`SELECT organization_id::text FROM locations WHERE id = $1`, locID,
		).Scan(&locOrgID)
	})
	if err != nil {
	...
	otherLocID := seedLocation(t, pool, otherOrgID, "Other Loc "+suffix)

...
func (s *Store) RecordSignIn(ctx context.Context, userID string) {
	_, _ = s.pool.Exec(ctx, `UPDATE auth_users SET last_sign_in_at = now() WHERE id = $1`, userID)
}
...
VALUES ($1, $2, now() + $3::interval, $4)
RETURNING id
`, userID, newHash, ttl.String(), nullString(userAgent)).Scan(&newID); err != nil {
		return "", err
	}
	if _, err := tx.Exec(ctx, `

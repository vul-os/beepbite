...
func (s *Store) GetSessionDetail(ctx context.Context, sessionID string) (*SessionDetail, error) {
	...
	err := db.Scoped(ctx, s.pool, db.ScopeFromContext(ctx), func(tx pgx.Tx) error {
		if err := scanSession(tx.QueryRow(ctx,
			`SELECT `+sessionCols+` FROM table_sessions WHERE id = $1`, sessionID,
		), &out.TableSession); err != nil {
			return err
		}

		seats, err := listSeatsTx(ctx, tx, sessionID)
		if err != nil {
			return err
		}
		out.Seats = seats
...
func (s *Store) CreateSeat(
	...
	var out Seat
	err := db.Scoped(ctx, s.pool, db.ScopeFromContext(ctx), func(tx pgx.Tx) error {
		return scanSeat(tx.QueryRow(ctx, `
...
func (s *Store) ListSeats(ctx context.Context, sessionID string) ([]Seat, error) {
	...
	err := db.Scoped(ctx, s.pool, db.ScopeFromContext(ctx), func(tx pgx.Tx) error {
		var err error
		out, err = listSeatsTx(ctx, tx, sessionID)
		return err
	})
	return out, err
}

func listSeatsTx(ctx context.Context, tx pgx.Tx, sessionID string) ([]Seat, error) {
	rows, err := tx.Query(ctx,
...
func (s *Store) UpdateSeat(
	...
	err := db.Scoped(ctx, s.pool, db.ScopeFromContext(ctx), func(tx pgx.Tx) error {
		return scanSeat(tx.QueryRow(ctx, `
UPDATE seats
SET seat_number = CASE WHEN $2::int IS NOT NULL THEN $2::int ELSE seat_number END,
...
func (s *Store) DeleteSeat(ctx context.Context, seatID string) error {
	...
	err := db.Scoped(ctx, s.pool, db.ScopeFromContext(ctx), func(tx pgx.Tx) error {
		ct, err := tx.Exec(ctx, `DELETE FROM seats WHERE id = $1`, seatID)
		if err != nil {
			return err
		}

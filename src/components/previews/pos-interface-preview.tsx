...
// Deliberately not `MenuItem & { quantity }`: the animation-scenario cart
// literals below are a separate, simplified demo dataset (id/name/price/
// image/quantity only) — they never carry inStock/popular, which only the
// menu grid uses.
interface CartItem {
  id: number;
  name: string;
...
type ScenarioStatus = 'adding' | 'checkout' | 'payment' | 'completed';
...
const POSInterfacePreview = ({ className }: { className?: string }) => {

import { useState, useMemo, type ReactNode, type ComponentType, type MouseEvent } from 'react';
...
export const usePrevNext = (pathname: string) => {
...
const Sidebar = ({ pathname, onItemClick = () => {} }: SidebarProps) => {
  ...
  const handleClick = (item: DocsNavItem) => (e: MouseEvent) => {
...
const DocsLayout = ({ children, title, description, hideSidebar = false }: DocsLayoutProps) => {
  const navigate = useNavigate();

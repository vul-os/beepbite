...
interface TrackingMapProps {
  store: { lat?: number | null; lng?: number | null; name?: string } | null;
  delivery: { lat?: number | null; lng?: number | null; label?: string | null } | null;
...
function svgIcon(svg: string, size: [number, number] = [28, 28], anchor: [number, number] = [14, 28]) {
  return L.divIcon({
    html: svg,
    iconSize: size,
    iconAnchor: anchor,
...
export default function TrackingMap({ store, delivery, driver }: TrackingMapProps) {
  const allPoints = React.useMemo(() => {
    const pts: MapPoint[] = [];
    if (store?.lat != null && store?.lng != null)       pts.push({ lat: store.lat, lng: store.lng, name: store.name });
    if (delivery?.lat != null && delivery?.lng != null) pts.push({ lat: delivery.lat, lng: delivery.lng, label: delivery.label ?? undefined });
    if (driver?.lat != null && driver?.lng != null)     pts.push({ lat: driver.lat, lng: driver.lng });
    return pts;
  }, [store, delivery, driver]);

  // Fallback centre: store, or Cape Town
  const initialCenter: [number, number] = store?.lat != null && store?.lng != null
    ? [store.lat, store.lng]
    : [-33.9249, 18.4241];

...
| **Multi-currency consolidated reporting** | `FX_PROVIDER=openrate` + `FX_OPENRATE_URL` (your own [OpenRate](https://github.com/vul-os/openrate) instance), or `FX_PROVIDER=openrate-embedded` to link the engine in-process and skip the HTTP hop entirely. No third-party FX API is embedded either way. | Nothing when unset. In `openrate` mode, requests to the instance you named. In `openrate-embedded` mode, this server fetches the rate sources itself — `FX_OPENRATE_SOURCES` is the complete list of hosts it will contact. | The consolidated report groups by currency instead of totalling — arguably more honest anyway. |

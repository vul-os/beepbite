...
CREATE TABLE payment_methods (
    ...
    code text NOT NULL UNIQUE, -- 'cash', 'card', 'eft', 'paystack', 'yoco', 'digital_wallet'
    name text NOT NULL, -- 'Cash', 'Card', 'EFT', 'PayStack', 'Yoco', 'Digital Wallet'
    is_active boolean DEFAULT true,
    requires_reference boolean DEFAULT false, -- Does this method need a reference number?
    supports_tips boolean DEFAULT true,
    processing_fee_percentage decimal(5,2) DEFAULT 0, -- Platform fee percentage
    fixed_fee_cents integer DEFAULT 0, -- Fixed fee in cents
    created_at timestamptz DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- Insert default payment methods (base definitions without fees)
INSERT INTO payment_methods (code, name, requires_reference, supports_tips) VALUES
('cash', 'Cash', false, true),
('card', 'Card', true, true),
('eft', 'EFT', true, false),
('zapper', 'Zapper', true, true),
...
('location-uuid', 'card', 0.65, 10000, 3.1, 10000),  -- We charge 0.65% + R1, PayStack charges 3.1% + R1
('location-uuid', 'eft', 0.0, 0, 0.0, 0),
('location-uuid', 'zapper', 2.0, 0, 2.5, 0),
('location-uuid', 'paystack', 0.65, 10000, 3.1, 10000);

-- Location B: Different rates
INSERT INTO location_payment_method_fees (location_id, payment_method_code, processing_fee_percentage, fixed_fee_cents, gateway_fee_percentage, gateway_fixed_fee_cents) VALUES
('location-uuid-2', 'cash', 0.0, 0, 0.0, 0),
('location-uuid-2', 'card', 1.0, 15000, 3.1, 10000),  -- Higher rates for this location
('location-uuid-2', 'eft', 0.5, 0, 0.0, 0),
('location-uuid-2', 'zapper', 2.5, 0, 2.5, 0),
('location-uuid-2', 'paystack', 1.0, 15000, 3.1, 10000);

-- Usage examples:
SELECT * FROM get_location_payment_methods('location-uuid');
SELECT * FROM calculate_payment_fees('location-uuid', 'card', 10000); -- R100 payment
*/ 
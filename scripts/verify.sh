#!/usr/bin/env bash
# =============================================================================
# scripts/verify.sh — verify a downloaded BeepBite release artifact, fail-closed.
#
...
#     name as a regex — every "." in "beepbite_v9.9.9_linux_amd64.tar.gz" is a wildcard
...
# USAGE
# ─────
#   scripts/verify.sh --tag v0.1.0 beepbite_v0.1.0_linux_amd64.tar.gz
#   scripts/verify.sh --tag v0.1.0 --attest beepbite_v0.1.0_linux_amd64.tar.gz
#   scripts/verify.sh --repo vul-os/beepbite --tag v0.1.0 --out ~/Downloads ASSET
...
trap cleanup EXIT
TMPDIR_V="$(mktemp -d "${TMPDIR:-/tmp}/beepbite-verify.XXXXXX")"
...
ART   = "beepbite_v9.9.9_linux_amd64.tar.gz"
GOOD  = b"beepbite-server-dist-bytes\n" * 64
...
trap_sums = (line(ART + ".sig", OTHER) +
             line("beepbite_v9X9X9_linux_amd64.tar.gz", OTHER)).encode()
...
run_selftest() {
  ...
  local art="beepbite_v9.9.9_linux_amd64.tar.gz"
  ...
  expect_exit "--attest but no gh installed"  "$E_ATTEST"         yes -- env PATH="$fakebin" "$bash_abs" "$SELF_PATH" --out "$outd" --attest --repo vul-os/beepbite --base-url "$base/good" "$art"

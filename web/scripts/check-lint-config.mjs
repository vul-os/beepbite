 ...
 *      no-unsafe-* family) are wired up or entirely absent. beepbite was one
 *      of the 12 — it linted 352 files and enforced nothing type-aware.
...
const CONFIG = {
  ...
  // resolves. beepbite's web/tsconfig.json has `"include": ["src/**/*"]`, so
  // "src" is covered and is where eslint.config.js's TS-aware block
  // (files: ['**/*.{ts,tsx}']) with recommendedTypeChecked applies.
  fixtureDir: 'src',
  fixtureName: '__lint_config_gate_probe.ts',

  // Repo root to walk for package.json files (assertion B). This reaches
  // outside webRoot on purpose — a stray range on any package.json in the
  // repo is worth catching even though only web/ depends on typescript today.
  ...
  // 631 warnings (before this script itself was added). 250 leaves headroom
